...
## Running a scan and generate a conf file 

```sh
docker run --user $(id -u):$(id -g) -w /zap -v $(pwd):/zap/wrk:rw --rm softwaresecurityproject/zap-stable:2.14.0 zap-baseline.py -t https://prod-oiahxczk.lab.practical-devsecops.training -g default.conf
```

## Config file 

With this config file, you make changes on your results whether it is in Inscope or scope.. Then even you can update the status in it 
...
## Scan with changes in configfile 

```sh
docker run --user $(id -u):$(id -g) -w /zap -v $(pwd):/zap/wrk:rw --rm softwaresecurityproject/zap-stable:2.14.0 zap-baseline.py -t https://prod-oiahxczk.lab.practical-devsecops.training -g default.conf -J zap-output.json
```